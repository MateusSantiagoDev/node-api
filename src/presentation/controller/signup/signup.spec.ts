import { InvalidParam, MissingParam, ServerError } from '../../error'
import { EmailValidator, AddAccount, AccountDto, AccountModel, HttpRequest, Validation } from './signup-protocols'
...
describe('SignUp Controller', () => {
  test('Should return 400 if no name is provided', async () => {
    ...
    expect(httpResponse).toEqual(badRequest(new MissingParam('name')))
  })
  ...
  test('Should return 400 if no confirmPassword is provided', async () => {
    ...
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse).toEqual(badRequest(new MissingParam('confirmPassword')))
  })
  ...
  test('Should return 400 if password confirmation fails', async () => {
    ...
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse).toEqual(badRequest(new InvalidParam('confirmPassword')))
  })
