...
import { EmailValidator, AddAccount, AccountDto, AccountModel } from './signup-protocols'
...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if no confirmPassword is provided', async () => {
    ...
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse.statusCode).toBe(400)
    expect(httpResponse.body).toEqual(new MissingParam('confirmPassword'))
  })
  ...
  test('Should return 400 if password confirmation fails', async () => {
    ...
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse.statusCode).toBe(400)
    expect(httpResponse.body).toEqual(new InvalidParam('confirmPassword'))
  })
